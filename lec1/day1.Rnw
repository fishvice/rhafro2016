...
\frametitle{Session info}
  \begin{columns}
  \begin{column}{0.5\linewidth}
  \bi \textbf{Environment}: Contains a list of all declared variables. If you have a dataset you can double click the variable to view the data. One can also use the \texttt{ls} command to find a list of variables in the environment
  \vs
  \vs
...
\vs
\item Define a function that calculates the condition factor of a whales using the formula:
$$C = 100*(W/L^3)$$
\item Use the function to calculate the condition factor for all whales that have ungutted weight
\vs
...
\begin{tikzpicture}
\node at (-2,2) {\includegraphics[width=0.5\linewidth]{rmd-code1.png}};
\node at (2,-2) {\includegraphics[width=0.5\linewidth]{rmd-eval.png}};
\node at (2,2) {\includegraphics[width=0.5\linewidth]{rmd-echo.png}};
\node at (-2,-2) {\includegraphics[width=0.5\linewidth]{rmd-echo2.png}};
\end{tikzpicture}
...
\bi Plot the location of
\item Create a markdown document that
\ei
}
...
R:
\bi R frá grunni (Háskólafjölritun)
\ei
Markdown:
\bi http://shiny.rstudio.com/articles/rmarkdown.html
\item http://www.introductoryr.co.uk/Reproducibility/Markdown\_guide.html
\ei
