...
@
\item If minke whales are in folder called data within the working directory:
<<eval=FALSE>>=
minke <- read.csv2('data/minke.csv')
@
\item If it is somewhere on the computer one can use absolute positioning:
<<eval=FALSE>>=
minke <- read.csv2('~/data/minke.csv') ## linux/mac
minke <- read.csv2('c:/data/minke.csv') ## windows
@
\item If it is on-line:
<<eval=FALSE>>=
minke <-
...
\frame{
\frametitle{Location of files -- continued}
\begin{columns}
